...
namespace DIY.AjaxResponse
{
    ...
    public class tech_mobileHandler : PageBaseHandler
    {
        ...
        private void SaveMeetingAdd()
        {
            ...
            if(meeting.enddate < meeting.begindate)
            {
                sbContextWrite.Append("错误！会议结束日期比开始日期早！");
                return;
            }
        ...
        private void SaveSkinAndSiteImg()
        {
            ...
            if (result >= 1)
            {
                ...
                string content = WebCommon.GetCookie(WebCommon.MEETING_KEY, 0).Split('=')[1] + "自助微站模板已开通！";
        ...
        private void SaveSiteTemplate()
        {
            ...
            if (i >= 1)
            {
                string content = WebCommon.GetCookie(WebCommon.MEETING_KEY, 0).Split('=')[1] + "自助微站模板已更新！";
                operating_record(content);
                sbContextWrite.Append("OK");
            }
            else
        ...
        private void SaveMobileImgs()
        {
            ...
            if (result >= 1)
            {
                string content = WebCommon.GetCookie(WebCommon.MEETING_KEY, 0).Split('=')[1] + "自助微站图片上传更新！";
                operating_record(content);
                sbContextWrite.Append("OK");
            }
            else
        ...
        private void show_moban(string mtype_id)
        {
            StringBuilder sb = new StringBuilder();
            tech_mobile_template info = tech_mobile_templateManager.Instance.GetModelByMTemplateId(mtype_id);
            sb.AppendFormat("<img src=\"{0}\" style=\"width:360px; height:657px;\" />", info.mtemplate_thumbnail);
        ...
        private void InputTechMTemplateList2(int pageIndex, int pageSize)
        {
            ...
            sb.Append(Page(pageCount, pageIndex, pageSize, "AjaxSubmitDiv", "tech_mobileHandler", 2, "form_kaitonging", "tb_Content"));
            sb.Append("</div>");
            response.Write(sb.ToString());
        }
