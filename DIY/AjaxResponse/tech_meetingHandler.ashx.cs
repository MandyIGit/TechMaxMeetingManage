...
namespace DIY.AjaxResponse
{
    ...
    public class tech_meetingHandler : PageBaseHandler
    {
        ...
        private void meeting_list(int pageIndex, int pageSize)
        {
            ...
            if (Convert.ToInt32(requst.Form["is_weizhankaitong"]) > 0)
            {
                info.is_weizhankaitong = Convert.ToInt32(requst.Form["is_weizhankaitong"]);
            }
            ...
            sb.Append("<th>会议状态</th>");
            sb.Append("<th>后台开通状态</th>");
            ...
            if (dt != null && dt.Rows.Count != 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string HyStatus = "";
                    //if (dt.Rows[i]["is_weizhankaitong"].ToString() == "1")
                    //{
                    //    if (DateTime.Parse(dt.Rows[i]["begindate"].ToString()) > DateTime.Now)
                    //    {
                    //        HyStatus = "<span class=\"org\">即将召开</span>";
                    //    }
                    //    else if (DateTime.Parse(dt.Rows[i]["begindate"].ToString()) <= DateTime.Now && DateTime.Parse(dt.Rows[i]["enddate"].ToString()) >= DateTime.Now)
                    //    {
                    //        HyStatus = "<span class=\"red\">正在召开</span>";
                    //    }
                    //    else if (DateTime.Parse(dt.Rows[i]["enddate"].ToString()) < DateTime.Now)
                    //    {
                    //        HyStatus = "<span class=\"green\">已结束</span>";
                    //    }
                    //}
                    //else
                    //{
                    //    HyStatus = "<span>申请中</span>";
                    //}
                    if (DateTime.Parse(dt.Rows[i]["begindate"].ToString()) > DateTime.Now)
                    {
                        HyStatus = "<span class=\"org\">即将召开</span>";
                    ...
                    {
                        HyStatus = "<span class=\"red_custom\">正在召开</span>";
                    }
                    else if (DateTime.Parse(dt.Rows[i]["enddate"].ToString()) < DateTime.Now)
                    {
                        HyStatus = "<span class=\"green\">已结束</span>";
                    ...
                    sb.AppendFormat("<td>{0}</td>", HyStatus);
                    sb.AppendFormat("<td>{0}</td>", dt.Rows[i]["is_weizhankaitong"].ToString() == "1" ? "已开通" : "申请中");
                    sb.AppendFormat("<td title=\"{0}\"><a href=\"{2}\" target=\"_blank\">{1}</a></td>", dt.Rows[i]["mname"].ToString(), Common.StringHelper.FirstString(dt.Rows[i]["mname"].ToString(), 60, "..."), dt.Rows[i]["m_website"].ToString());
                    ...
                    sb.Append("<td>");
                    sb.AppendFormat("<a href=\"tech_meeting_edit.aspx?mid={0}\" class=\"btn btn-primary btn-xs\">修改会议信息</a>", dt.Rows[i]["mid"].ToString());
                    sb.Append("&nbsp;");
                    sb.AppendFormat("<a href=\"javascript:;\" class=\"btn btn-success btn-xs\" onclick=\"ManageMeeting('{0}')\">微站设置管理</a>", DESEncrypt.Encrypt(dt.Rows[i]["mid"].ToString() + "|" + dt.Rows[i]["zzjzpasswd"].ToString()));
                    sb.Append("</td>");
                    sb.Append("</tr>");
                }
            }
