 ...
 function Get_Msg(msgid){       
       ...
       $.ajax({
          type: "post",
          url: "/AjaxResponse/conference_userHandler.ashx?type=GetReciveMsg&msg_id="+msgid,            
...
function MakeItRead(msgid){
   $.ajax({
       type: "post",
       url: "/AjaxResponse/conference_userHandler.ashx?type=MakeMsgRead&msg_id="+msgid,            
       beforeSend:function(XMLHttpRequest, textStatus){ },                  
       success: function(data) { },
       error: function(XMLHttpRequest, textStatus, errorThrown) {alert(' 发生错误，请联系客服！ ');},
 ...
 function Get_SendMsg(msgid){ 
       ...
       $.ajax({
          type: "post",
          url: "/AjaxResponse/conference_userHandler.ashx?type=GetSendMsg&msg_id="+msgid,            
          beforeSend:function(XMLHttpRequest, textStatus){ },                  
          success: function(data) { 
               var objJSON = eval("(" + data + ")"); 
 ...
 function SendMsg(){ 
     if(!CheckEmpty()){  return;}  
       $.ajax({
          type: "post",
          url: "/AjaxResponse/conference_userHandler.ashx?type=Send_Msg",  
           data: $("#sendmsgform").serialize(),          
          beforeSend:function(XMLHttpRequest, textStatus){$("#btn_send").hide();$("#btn_sending").show(); },                  
          success: function(data) {               
              var objJSON = eval("(" + data + ")"); 
              if(objJSON.status=='succ'){alert(objJSON.msg); window.location.href="user_sendlist.aspx";
