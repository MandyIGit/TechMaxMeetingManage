<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="tech_html_template_list_list.aspx.cs" Inherits="WebSite.Admin.HtmlTemplatePage.tech_html_template_list_list" %>
            ...
            $('.mydel').click(function () {
                ...
                if (confirm('您确认要删除吗？')) {
                    $.ajax({
                        ...
                        type: 'GET',
                        url: '/AjaxResponse/tech_html_template_listHandler.ashx?type=del&id=' + deviceid,
                        success: function (data) {
                            var r = eval("(" + data + ")");
                            if (r.result == 'succ') {
                                alert(r.msg);
                                window.location.href = 'tech_html_template_list_list.aspx?page=<%=mypager.CurrentPageIndex%>';
                            }
                            else {
                                alert(r.msg);
                            }
            ...
            <asp:Repeater ID="rpt_listNews" runat="server">                
                <ItemTemplate>
                    <tr>
                        ...
                        <td><%#Eval("tm_type") %></td>
                        <td><%#Eval("tm_css") %></td>
                        <td><%#DateTime.Parse(Eval("inputtime").ToString()).ToString("yy年MM月dd日") %></td>
                        <td>
                            <a href="<%#"tech_html_template_list_edit.aspx?mid="+Eval("mid")+"&tm_id="+Eval("tm_id") %>&page=<%=mypager.CurrentPageIndex %>" class="btn btn-secondary btn-xs">编辑</a>
                            <!--<a deviceid='<%#Eval("tm_id") %>' class="btn btn-red btn-xs mydel" href="#">删除</a>-->
                        </td>
                    </tr>
                </ItemTemplate>
            </asp:Repeater>
