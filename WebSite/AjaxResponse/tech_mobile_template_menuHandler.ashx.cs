...
{
    /// <summary>
    /// tech_mobile_template_menuHandler 的摘要说明
    /// </summary>
    public class tech_mobile_template_menuHandler : PageBaseHandler
    {
        HttpRequest requst;
        ...
        {
            string mtemplate_id = requst.Form["mtemplate_id"].ToString();
            ...
            sb.Append("</tr>");
            sb.Append("</thead>");
            IList<tech_mobile_template_menu> list = tech_mobile_template_menuManager.Instance.GetMenuList(mtemplate_id);
            foreach (tech_mobile_template_menu item in list)
            {
                sb.Append("<tbody>");
        ...
        {
            int i = 0;
            string mtemplate_id = requst.Form["mtemplate_id"];
            string strjson = requst.Form["JsonStr"];

            JavaScriptSerializer json = new JavaScriptSerializer();
            List<tech_mobile_template_menu> list = json.Deserialize<List<tech_mobile_template_menu>>(strjson);
            foreach (tech_mobile_template_menu item in list)
            {
                if (item.menu_name != "请输入菜单名称")
                {
                    item.mt_id = Convert.ToInt32(mtemplate_id);
                    ...
                    i += tech_mobile_template_menuManager.Instance.ModifyMenu(item);
                }
            }
        ...
        private void DeleteMenu()
        {
            ...
            int i = tech_mobile_template_menuManager.Instance.Delete(menu_id);
            response.Write(i.ToString());
        }
