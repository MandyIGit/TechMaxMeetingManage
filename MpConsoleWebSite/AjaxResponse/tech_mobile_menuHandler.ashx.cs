...
namespace MpConsoleWebSite.AjaxResponse
{
    ...
    public class tech_mobile_menuHandler : PageBaseHandler
    {
        ...
        private void GetMenu()
        {
            ...
            sb.Append(" <td>菜单图标编码</td>");
            sb.Append(" <td>图标编码</td>");
            sb.Append(" <td>链接</td>");
            sb.Append(" <td>操作</td>");
            ...
            foreach (tech_mobile_menu item in list)
            {
                ...
                sb.AppendFormat("<td><input type=\"text\" name=\"sort\" id=\"sort_{0}\" value=\"{1}\" class=\"txt_table mr5 txt20\"></td>", item.Menu_id, item.Sort);
                sb.AppendFormat("<td><input type=\"text\" name=\"menu_name\" id=\"menu_name_{0}\" class=\"noborder mr5 txt80\" value=\"{1}\"></td>", item.Menu_id, item.Menu_name);
                sb.AppendFormat("<td><input type=\"text\" name=\"menu_icon\" id=\"menu_icon_{0}\" class=\"noborder mr5 txt80\" value=\"{1}\"></td>", item.Menu_id, item.Menu_icon);
                sb.AppendFormat("<td><xmp>{0}</xmp></td>", item.Menu_icon);
                sb.AppendFormat("<td><input type=\"text\" name=\"menu_url\" id=\"menu_url_{0}\" value=\"{1}\" class=\"txt_table\"></td>", item.Menu_id, item.Menu_url);
                sb.AppendFormat("<td><a class=\"mr5\" onclick=\"deletemenu('{0}')\">[删除]</a><a class=\"mr5\" onclick=\"dismenu('{0}')\">[{1}]</a></td>", item.Menu_id, item.Isban == 1 ? "导航显示" : "导航隐藏");
                sb.Append("<td>");
                if (i == 1)
                {
                ...
                if (i == list.Count)
                {
                    sb.AppendFormat("<button type=\"button\" disabled=\"disabled\" class=\"btn btn-primary\">下移</button>");
                }
                else
                {
                    sb.AppendFormat("<button type=\"button\" class=\"btn btn-primary\" onclick=\"menu_down('{0}','{1}')\">下移</button>", item.Menu_id, item.Sort);
                }
                sb.Append("</td>");
                sb.Append("</tr>");
                sb.Append(" </tbody>");
        ...
        private void GetMenuContent()
        {
            ...
            foreach (tech_mobile_menu item in list)
            {
                ...
                sb.AppendFormat("<td><input type=\"text\" value=\"menu_id={0}\" class=\"txt\"></td>", item.Menu_id);
                sb.Append("<td>");
                if (string.IsNullOrEmpty(item.Menu_url))
                    sb.Append("<a href=\"javascript:void(0)\" onclick=\"OpenUrl('mobile_menu/menu_content_edit.aspx?menuid=" + item.Menu_id + "','内容管理',850,555)\" class=\"mr5\">[管理内容]</a>");
                sb.Append("</td>");
                sb.Append("</tr>");
                sb.Append("</tbody>");
            }
