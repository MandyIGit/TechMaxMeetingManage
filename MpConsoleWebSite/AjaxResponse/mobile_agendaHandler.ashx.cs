    ...
    public class mobile_agendaHandler : PageBaseHandler
    {
        ...
        public void GetHallLists()
        {
            ...
            foreach (tech_meeting_hall item in list)
            {
                ...
                sb.AppendFormat("<td><a href=\"javascript:void(0)\" onclick=\"javascript:OpenUrl('/home/mobile_agenda/modifyhall.aspx?hid=" + item.Hallid + "','修改会议大厅',500,400);\"  class=\"i-xiugai\">修改</a></td>");
                sb.AppendFormat("<td><a href=\"javascript:void(0)\" onclick=\"javascript:deletehall('{0}')\"  class=\"i-shanchu\">修改</a></td>", item.Hallid);
                sb.Append("</tr>");
            }
            sb.Append("<tr>");
            sb.Append("<td colspan=\"5\" style=\"text-align:right;\"><a href=\"javascript:void(0)\" class=\"btnblue\" onclick=\"javascript:OpenUrl('/home/mobile_agenda/modifyhall.aspx','添加会议大厅',440,240);\">添加会议厅</a></td>");
            sb.Append("</tr>");
            response.Write(sb.ToString());
        }
        ...
        {
            StringBuilder sb = new StringBuilder();
            string family_name = Convert.ToString(request.Form["rolefamilyname"]);
            string given_name = Convert.ToString(request.Form["rolegivenname"]);
            if (!string.IsNullOrEmpty(family_name) || !string.IsNullOrEmpty(given_name))
            {
                ...
                foreach (DataRow dr in hallList.Rows)
                {
                    ...
                    if (sessionList != null)
                    {
                        foreach (DataRow sRow in sessionList.Rows)
                        {
                            ...
                            sb.Append("<td>");
                            sb.AppendFormat("<a href=\"javascript:void(0)\" onclick=\"OpenUrl('/home/mobile_agenda/add_sch.aspx?session_id={0}','增加日程',600,450)\">增加日程</a>&nbsp;| ", sRow["session_id"]);
                            sb.Append("&nbsp;<a href=\"javascript:void(0)\" onclick=\"OpenUrl('/home/mobile_agenda/schedule_list.aspx?session_id=" + sRow["session_id"] + "','修改会议日程',900,600)\">日程管理</a>&nbsp;|");
                            sb.Append("&nbsp;<a href=\"javascript:void(0)\" onclick=\"OpenUrl('/home/mobile_agenda/edit_session.aspx?session_id=" + sRow["session_id"] + "','单元修改',900,600)\">单元修改</a>");
                            sb.Append("</td>");
                            sb.Append("</tr>");
                        }
                    }
        ...
        protected void GetScheduleList()
        {
            ...
            if (dtResult != null)
            {
                foreach (DataRow dr in dtResult.Rows)
                {
                    ...
                    sb.AppendFormat("<td><a class=\"i-xiugai\" onclick=\"OpenUrl('/home/mobile_agenda/edit_sch.aspx?sch_id={0}&session_id={1}','修改日程',600,450)\" title=\"修改\">修改</a></td>", dr["agenda_id"], session_id);
